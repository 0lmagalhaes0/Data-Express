...
def nananan():
    ...
    if port == False:
        ...
        nan_method_list = ["Selecione",
                       "Apagar linhas que tenham células vazias/inválidas (recomendado em casos de oucas células vazias)",
...
if port == False:
    ...
    if st.checkbox("Me selecione para lidar com células vazias/inválidas"):
...
def get_table_download_link(data):
    if port == False:
        ...
        file_name = st.text_input("Escolha o nome do seu arquivo:")
...
def geograph():
    ...
    if port == False:
        ...
        if filtering == True:
            ...
            st.warning("Se seu filtro for Boolean (True/False), por favor converta-o para o tipo Texto (Object) antes.")
            for i in filter:
                data = data.loc[data[map_filter] == filter]
        data.rename(columns={latitude:"latitude",longitude:"longitude"},inplace=True)
...
def ml():
    if port == False:
        ...
        st.write("Obrigado por utilizar este aplicativo até aqui. Esta parte aida está em desenvolvimento, entretanto. Por favor "
            "tente de novo em alguns dias.")
    return ""
